...
VOCAB = SPECIAL_TOKENS + ELEMENTS + [
    WORD_GREATER, WORD_LESSER, "is", "than", ".", ",", 
...
# ==========================================
# 3. データ作成関数 (English Prompt & Labels)
# ==========================================
def generate_sample():
    perm = ELEMENTS.copy()
    random.shuffle(perm)
    ...
    for e1, e2 in pairs:
        ...
        # 英語の文法に合わせた構成: "A is greater than B ."
        prompt.extend([first, "is", word, "than", second, "."])
        
    # 指示文: "Sort in descending order ."
    prompt.extend(["Sort", "in", "descending", "order", "."])
    
    # 応答文（正解の降順）: "A , B , C ."
    response = [perm[0], ",", perm[1], ",", perm[2], "."]
    
    full_tokens = prompt + ["<SEP>"] + response + ["<EOS>"]
    input_ids = [word2idx[w] for w in full_tokens]
    
    # ターゲット（正解ラベル）の作成: プロンプト部分は Loss 計算から除外 (-100)
...
print("Starting training of Causal Transformer LM (English)...")
model.train()
...
# ==========================================
print("\n" + "="*50)
print("--- Inference Test (Text Generation) ---")
print("="*50)

test_prompts = [
    ["A", "is", "greater", "than", "B", ".", "B", "is", "smaller", "than", "C", ".", "C", "is", "smaller", "than", "A", ".", "Sort", "in", "descending", "order", "."],
    ["A", "is", "smaller", "than", "B", ".", "B", "is", "greater", "than", "C", ".", "C", "is", "smaller", "than", "A", ".", "Sort", "in", "descending", "order", "."],
    ["A", "is", "smaller", "than", "B", ".", "B", "is", "smaller", "than", "C", ".", "C", "is", "greater", "than", "A", ".", "Sort", "in", "descending", "order", "."]
]
